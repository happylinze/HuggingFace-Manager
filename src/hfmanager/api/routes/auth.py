...
def login(req: LoginRequest, auth_mgr: AuthManager = Depends(get_auth_manager)):
    """Login with HF Token."""
    success, message = auth_mgr.login_with_token(req.token)
    return {"success": success, "message": message}

@router.post("/logout", response_model=ActionResponse)
def logout(auth_mgr: AuthManager = Depends(get_auth_manager)):
    """Logout from HF."""
    success, message = auth_mgr.logout_user()
    return {"success": success, "message": message}
@router.get("/user")
def get_user_info(auth_mgr: AuthManager = Depends(get_auth_manager)):
...
@router.get("/accounts")
def get_accounts(auth_mgr: AuthManager = Depends(get_auth_manager)):
    """Get list of saved accounts."""
    return {"accounts": auth_mgr.get_known_accounts()}

@router.post("/switch", response_model=ActionResponse)
def switch_account(req: SwitchAccountRequest, auth_mgr: AuthManager = Depends(get_auth_manager)):
...
def remove_account(username: str, auth_mgr: AuthManager = Depends(get_auth_manager)):
